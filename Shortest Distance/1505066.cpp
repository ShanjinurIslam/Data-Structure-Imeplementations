...
bool Graph::BellmanFord(int source) {
    for(int i=1;i<nVertices;i++){
        vertexs[i].setDistance(INFINITY) ;
        vertexs[i].setParent(NIL) ;
    }
    vertexs[source].setDistance(0) ;
    vertexs[source].setParent(NIL) ;
    for(int i=1;i<nVertices-1;i++){
        for(int j=0;j<nEdges;j++){
            int u = edges[j][0] ;
            int v = edges[j][1] ;
            if(vertexs[v].getDistance()>edgeWeight[u][v]+vertexs[u].getDistance()){
                vertexs[v].setDistance(vertexs[u].getDistance()+edgeWeight[u][v]) ;
                vertexs[v].setParent(u) ;
            }
        }
    }
